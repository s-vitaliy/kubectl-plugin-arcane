...
func ProvideStreamClassDiscoveryService(logger *slog.Logger) abstractions.ApiSettingsDiscoverer {
	if logger == nil {
		logger = slog.Default()
	}
	return &streamClassDiscoveryService{logger: logger}
}

// DiscoveryFromJobs discovers API settings from a job.
func (s *streamClassDiscoveryService) DiscoveryFromJobs(ctx context.Context, dynamicInterface dynamic.Interface, jobstreamClass string, namespace string) (*models.ClientApiSettings, error) {
	jobResourceRef := schema.GroupVersionResource{Group: "batch", Version: "v1", Resource: "jobs"}
	dynamicClient := dynamicInterface.Resource(jobResourceRef).Namespace(namespace)
...
func (s *streamClassDiscoveryService) DiscoveryFromStreamClass(ctx context.Context, dynamicInterface dynamic.Interface, streamClass string, namespace string) (*models.ClientApiSettings, error) {
	streamClassResourceRef := schema.GroupVersionResource{
		Group:    "streaming.sneaksanddata.com",
		Version:  "v1beta1",
		Resource: "stream-classes",
	}
	dynamicClient := dynamicInterface.Resource(streamClassResourceRef).Namespace(namespace)
